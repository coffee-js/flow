...
combinator = do ->
  ws = (p) ->
    pc.map pc.seq(pc.ws(p), pc.choice pc.space(), pc.end()), (n)->n[0]
  ...
  word = pc.map pc.and(pc.rep1(pc.neg pc.space()), pc.neg(ws(pc.ch '[]')), pc.neg(name)),
    (n) -> new ast.NodeWord n.reduce (t,s)->t.concat(s)

  elem = null
  _elem = pc.lazy ->elem

  seq = pc.rep1 ws _elem
  block = pc.map pc.seq(pc.tok('['), pc.space(), pc.optional(seq), pc.tok(']')),
  ...
  { ws, int10, number, string, colon, negws, nameChar, name, word, elem, seq, block, value }
