...
desc 'compile #{SrvJsDir}/app/ from spec/'
task :compile_spec => src do
  sh "#{CoffeeCmd} -c -o #{SrvJsDir}/app/ spec/spec.coffee"
end
...
desc 'run auto spec test in browser'
task :auto => %w[compile_spec] do
  sh "node spec.js"
end
